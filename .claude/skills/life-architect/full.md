# Life Architect: full setup, retune, one area

Loaded only for "full setup", "re-tune" / "my life changed", or one area.
The quick start and the one-question-a-day flow live in `SKILL.md`; its
"Never" list applies here too. Read `AGENTS.md` first: its rules on shame,
lane, crisis and outward actions apply in full.

- `full`: Phases 0–7 below, about 30–45 minutes, pausable. Some people,
  often autistic people, prefer one structured session.
- `area <name>`: Phases 4–6 scoped to that area.
- `retune`: see "Retune mode".

References: `question-bank.md` (questions per phase; pick, never read out),
`patterns.md` (patterns, neurodivergent adaptations, alternatives),
`topics.md` (per-topic notes, including where each answer may be stored).
