...
0. **One read:** `./life review` (done items, this week's log, inbox, each
   area's focus, therapy-note count).
1. **Wins first,** from that packet. Show them as a short list. Thin list? Find real ones in the log;
   getting through a hard week counts.
2. **Empty the Inbox.** For each item propose Now /
...
3. **Areas and load.** One line per area in the packet: still the right
...
5. **Next week.** At most 3 focus items, plus fixed commitments that change
   ...
   what's loud?"
   - Therapy notes: if the packet's count is more than 0, say only "N things
     noted for your therapist". Skip it entirely if their notes say others
     see their screen. Never read the notes.
   - Finish, one call: `./life reviewed --focus "<a>" [--focus "<b>"]
     [--win "<a win from the log>"]`. It logs the review, clears Done this
